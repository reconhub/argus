...
unlink(config$epidemiological_report_plots_paths)

# Load shapefiles ####
sp_files <- st_read(paste0(config$assets_path, config$shape_files), quiet=T)
...
plot_occurrence <- ggplot(data = diseaseThreshold_W12,aes(x = week, y = occurence, group = variable, color = variable)) +
  ...
  theme(title = element_text(face="bold", size = 16),
    ...
    strip.text.x = element_text(size = main_font_size, family = "sans",hjust = 0.5),
    strip.background=element_rect(fill = "white", colour = "white"),
    panel.border = element_blank())

ggsave(file = paste0(config$assets_path, "trends_occurrence.svg"), plot = plot_occurrence, width = 10, height = 9)

# Create maps ####
# to do: when districts to close to each others, group the nb of cases on one location
...
if(nrow(diseaseThreshold_W2)==0) {
  ...
  mapTemp <-  ggplot() +
      ...
      geom_point(data = disease_location[which(disease_location$diseaseName==diseasesMap[i]),], aes(x = longitude, y = latitude, size = occurence), color = config$plot_colors[1], alpha = 1) +
