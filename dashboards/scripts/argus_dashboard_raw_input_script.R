...
weekFunction <- function(x) {
  if(config$weekFirstDay==1) {
    return(isoweek(x))
  } else {
    if(config$weekFirstDay==7) {
      return(epiweek(x))
    } else {
...
dateStart_W12 <- floor_date(today()-weeks(12),unit="week", week_start = config$weekFirstDay)
dateStart_W8 <- floor_date(today()-weeks(8),unit="week", week_start = config$weekFirstDay)
dateStart_W3 <- floor_date(today()-weeks(3),unit="week", week_start = config$weekFirstDay)

if(weekFunction(floor_date(today(),unit = "year"))==1) {
  dateStart_YR <- floor_date(floor_date(today(),unit = "year"),unit="week",week_start = config$weekFirstDay)
} else {
  dateStart_YR <- floor_date(floor_date(today(),unit = "year")+7,unit="week",week_start = config$weekFirstDay)
} 

if(weekFunction(floor_date(today()-years(1),unit = "year"))==1) {
  dateStart_YR_previous <- floor_date(floor_date(today()-years(1),unit = "year"),unit="week",week_start = config$weekFirstDay)
} else {
  dateStart_YR_previous <- floor_date(floor_date(today()-years(1),unit = "year")+7,unit="week",week_start = config$weekFirstDay)
} 
...
dateEnd <- floor_date(today()-weeks(1),unit="week", week_start = config$weekFirstDay)

dateEnd_YR_previous <- floor_date(today()-weeks(1) - years(1),unit="week", week_start = config$weekFirstDay)
...
if(weekEnd>weekEnd_YR_previous) {
  dateEnd_YR_previous <- floor_date(today() - years(1),unit="week", week_start = config$weekFirstDay)
} else {
  if(weekEnd < weekEnd_YR_previous) {
    dateEnd_YR_previous <- floor_date(today()-weeks(2) - years(1),unit="week", week_start = config$weekFirstDay)
  } else {
  }
}
...
baseSql <- dbConnect(MySQL(), user = db_config$DB_USER,
